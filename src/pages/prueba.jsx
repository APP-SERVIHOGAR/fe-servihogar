...
function EditarPerfilPage() {
  ...
  useEffect(() => {
    const cargarPerfil = async () => {
      try {
        ...
        // Si hay provincia, cargar las localidades primero
        if (provinciaId) {
          const locRes = await fetch(`http://localhost:3000/localidad?id_provincia=${provinciaId}`);
          const locData = await locRes.json();
          setLocalidades(locData);
        }
        
        // Ahora sí setear el form con todos los datos
        setFormData({
          nombre: data.nombre || '',
          ...
          id_provincia: data.localidad?.id_provincia || '',
  ...
  // Cargar localidades cuando el usuario CAMBIA la provincia manualmente
  useEffect(() => {
    if (formData.id_provincia && !loading) {
      fetch(`http://localhost:3000/localidades?id_provincia=${formData.id_provincia}`)
        .then(res => res.json())
        .then(data => setLocalidades(data))
        .catch(err => console.error(err));
    }
  }, [formData.id_provincia, loading]);
